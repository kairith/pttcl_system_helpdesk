...
export async function POST(request: Request) {
  ...
  try {
    ...
    ) as { isAdmin: boolean; rules_id: number };
    if (!decoded.isAdmin) {
      return NextResponse.json(
        { error: "Admin access required: Invalid role or permissions" },
        { status: 403 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: "All fields (except imagePath) are required" },
        { status: 400 },
      );
    }
    if (!/^\S+@\S+\.\S+$/.test(email)) {
    ...
    if (error.code === "ER_NO_SUCH_TABLE") {
      return NextResponse.json(
        { error: `Table not found: ${error.sqlMessage}` },
        { status: 500 },
      );
    }
    ...
    if (error.code === "ER_NO_REFERENCED_ROW_2") {
      return NextResponse.json(
        { error: "Invalid rules_id: Role does not exist" },
        { status: 400 },
      );
    }
    return NextResponse.json(
