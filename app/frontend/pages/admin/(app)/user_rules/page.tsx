...
export default function UserRules() {
  ...
  const { rules: rawRules, authLoading } = useUserData();
  ...
  const permissions = useMemo<Permissions | null>(() => {
    ...
    return {
      users: {
        add: !!rawRules.add_user_status,
        edit: !!rawRules.edit_user_status,
        delete: !!rawRules.delete_user_status,
        list: !!rawRules.list_user_status,
      },
      tickets: {
        add: !!rawRules.add_ticket_status,
        edit: !!rawRules.edit_ticket_status,
        delete: !!rawRules.delete_ticket_status,
        list: !!rawRules.list_ticket_status,
        listAssign: !!rawRules.list_ticket_assign,
      },
      stations: {
        add: !!rawRules.add_station,
        edit: !!rawRules.edit_station,
      ...
      userRules: {
        ...
        delete: !!rawRules.delete_user_rules,
        list: !!rawRules.list_user_rules,
      },
    };
  }, [rawRules]);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setError(null);
      try {
        if (!currentPermissions.userRules.list) {
  ...
  const handleEdit = useCallback((rule: tbl_users_rules) => {
    ...
    if (rule.add_user_rules === 1) {
      toast.error("Cannot edit an admin role.");
      return;
    }
  ...
  const handleEditSubmit = useCallback(async () => {
    ...
    if (selectedRule.add_user_rules === 1) {
      toast.error("Cannot edit an admin role.");
      return;
    }
  ...
  const handleDelete = useCallback(async () => {
    ...
    if (selectedRule.add_user_rules === 1) {
      toast.error("Cannot delete an admin role.");
    ...
    <>
      ...
      <Card className="p-4 sm:p-6">
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          {/* user rules permissions implementation */}
          {permissions.userRules.add && (
            <Link href="/pages/admin/user_rules/add_rules">
              ...
              >
                <span className="mr-2">+</span> Add Rules
              </button>
            </Link>
          )}
        </div>
              ...
              {showFilterInput && (
                  ...
                  {filteredRules.length === 0 ? (
                    ...
                    <div className="overflow-x-auto w-full max-w-full min-w-0">
                      <table
                        className="w-full max-w-full min-w-0 text-sm table-auto"
                            ...
                            >
                              ...
                              {(permissions?.userRules?.edit || permissions?.userRules?.delete) && (
                                <td className="p-2 sm:p-3 text-gray-700 min-w-0 flex gap-2">
                                  {permissions?.userRules?.edit && row.add_user_rules !== 1 && (
                                    <button
                                  ...
                                  {permissions?.userRules?.delete && row.add_user_rules !== 1 && (
                                    <button
                                      onClick={() => {
                                        setSelectedRule(row);
