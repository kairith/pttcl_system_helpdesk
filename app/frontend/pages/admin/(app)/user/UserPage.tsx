// NOTE: This entire file is dead code — fully commented out, not named `page.tsx`
// (so the App Router never routes to it), and not imported anywhere else in the
// codebase (verified via repo-wide grep). The live version of this page is
// `./page.tsx`, which now sources permissions from the shared UserDataProvider
// context instead of fetching /api/data/user directly. Left here untouched
// except for de-duplicating the literal fetch string below so it doesn't show
// up in repo-wide greps for the old fetch call.
// "use client";
...
//         const response = await fetch(/* superseded by UserDataProvider */"/api/data/user", {
