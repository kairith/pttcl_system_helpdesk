...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!userId) {
      return NextResponse.json({ error: 'Invalid token: userId missing (tried users_id, userId, id, sub)' }, { status: 401 });
    }
    ...
    // Generate ticket_id (e.g., POS250600004)
    const now = new Date();
    const year = now.getFullYear().toString().slice(-2); // YY (e.g., 25)
    const month = (now.getMonth() + 1).toString().padStart(2, '0'); // MM (e.g., 06)
    const dateStr = `${now.getFullYear()}-${month}-${now.getDate().toString().padStart(2, '00')}`; // YYYY-MM-DD

    // Count tickets for today to get sequence number
    const [countRows] = await connection.execute(
      'SELECT COUNT(*) as count FROM tbl_ticket WHERE DATE(ticket_open) = ?',
      [dateStr]
    );
    const sequence = ((countRows as any)[0].count + 1).toString().padStart(6, '0'); // NNNNN
    const ticketId = `POS${year}${month}${sequence}`;
